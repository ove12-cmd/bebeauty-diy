...
export default function ShopPage() {
  ...
  const basePrice = codeApplied ? Math.round(variant.price * (1 - DISCOUNT)) : variant.price;
  ...
  return (
    <main className="bb-shop">
      ...
      <StickyBar price={`${finalPrice}€`} original={`${variant.original}€`} onAdd={addToCart} />
          ...
          <div className="bb-shop__variants">
            {VARIANTS.map(v => (
              <button key={v.id} className={`bb-shop__variant ${selected === v.id ? "bb-shop__variant--active" : ""}`} onClick={() => setSelected(v.id)}>
                ...
                <span className="bb-shop__variant-price">{v.price}€</span>
              </button>
            ))}
          </div>
