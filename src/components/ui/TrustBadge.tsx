...
export default function TrustBadge({ className = "" }: { className?: string }) {
  return (
    ...
    >
      <Stars rating={AVERAGE_RATING} label={`Average rating ${formatRating(AVERAGE_RATING)} out of 5`} />
      <span>
        {formatRating(AVERAGE_RATING)} · {REVIEW_COUNT} reviews{" "}
        <span className="text-[var(--bb-gold-deep)] underline">(see)</span>
      </span>
