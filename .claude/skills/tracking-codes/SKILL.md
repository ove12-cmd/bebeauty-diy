...
2. **The reference list** — `src/lib/tracking-scripts.ts`, a `TRACKING_SCRIPTS: TrackingScript[]` array (`id`, `name`, `location`, `source`, `code`, optional `verifiedAt`, optional `events: TrackingEvent[]`). **Adding an entry here does NOT make it run** — it's display-only. Next.js won't execute a `<script>` string injected via `dangerouslySetInnerHTML` on a plain element (only `next/script` actually runs), so there is no way to make this list double as the live injector without rewriting the whole approach. The two have to be kept in sync by hand.
3. **The admin page** — `src/app/dashboard/tracking-codes/page.tsx`, gated behind the same `verifySession()` cookie auth as the orders dashboard (`/dashboard/login`). Read-only: shows each script's full code, its file location, an "✓ Verified <date>" badge, which `<head>`/`<body>` region it belongs to, and — when the script fires more than one event — a "What this is tracking" table (event name, what triggers it, whether it's browser-only or also mirrored server-side). Linked from the main dashboard header.
...
3. **If the script fires more than an implicit PageView (Meta Pixel, GA4, any conversion pixel), add an `events` array** — one `TrackingEvent` (`name`, `firesFrom`, `client`, `server`) per event it actually sends. This is what turns "here's the code" into "here's what it's actually doing" — the owner explicitly asked for this once already (don't make them ask again for the next script). Meta Pixel's entry is the reference example: 5 events, each naming the exact file that fires it and whether it's browser-only or also CAPI-mirrored.
4. **Verify it before marking it verified** — see "How to verify" below. Only set `verifiedAt` once you've actually confirmed the global (`window.gtag`/`window.fbq`/`window.clarity`/etc.) initializes. Don't take the snippet's presence as proof it works.
5. Commit and push to `main` — this repo has no staging branch or PR flow; Vercel auto-deploys straight from `main` (confirmed via `git remote -v` → `github.com/ove12-cmd/bebeauty-diy`, and the commit history is all direct-to-main).
